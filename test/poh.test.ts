...
describe("ProofOfHumanity", () => {
  beforeEach("Initializing the contracts", async () => {
    [governor, requester, requester2, challenger1, challenger2, voucher1, voucher2, voucher3, other] =
    ...
    await poh.connect(governor).addSubmissionManually(voucher1.address, startingTimestamp);
    await poh.connect(governor).addSubmissionManually(voucher2.address, startingTimestamp);
    await poh.connect(governor).addSubmissionManually(voucher3.address, startingTimestamp);
  });
  ...
  it("Should set correct values in manually added submissions", async () => {
    expect(await poh.submissionCounter()).to.equal(3);
    ...
    await checkSubmissionInfo(voucher2.address).for({ registered: false });
    await poh.connect(governor).addSubmissionManually(voucher2.address, startingTimestamp);
    await checkSubmissionInfo(voucher2.address).for({ registered: true, status: Status.None });
    expect(await poh.submissionCounter()).to.equal(3);

    await poh.connect(governor).addSubmissionManually(other.address, startingTimestamp);
    expect(await poh.submissionCounter()).to.equal(4);
  });
  ...
  it("Should set correct values after creating a request to remove a submission", async () => {
    await expectRevert(
      poh.connect(requester).removeSubmission(voucher1.address, "evidence1", { value: requesterTotalCost.sub(1) }),
      "You must fully fund your side"
    );

    const txRemove = await (
      await poh.connect(requester).removeSubmission(voucher1.address, "evidence1", {
    ...
    const contribution = await poh.getContributions(voucher1.address, 0, 0, 0, requester.address);
    ...
    expectEvent(txRemove, 0)
      ...
      .with({ _requester: requester.address, _submissionID: voucher1.address, _requestID: 0 });
    ...
    await expectRevert(
      poh.connect(requester).removeSubmission(voucher2.address, "evidence1", { value: requesterTotalCost }),
  ...
  it("Should correctly store vouches and change vouching state", async () => {
    ...
    const txVouchAdd = await (await poh.connect(voucher1).addVouch(requester.address)).wait();

    expect(await poh.vouches(voucher1.address, requester.address), "Should register the vouch for the submission").to.be
      .true;
    ...
    const txVouchRemove = await (await poh.connect(voucher1).removeVouch(requester.address)).wait();

    expect(await poh.vouches(voucher1.address, requester.address), "The vouch should be removed").to.be.false;
    ...
    await expectRevert(
      poh.connect(governor).changeStateToPending(requester.address, [voucher1.address, voucher2.address], [], []),
    ...
    await poh
      ...
      .changeStateToPending(
        requester.address,
        [governor.address, voucher1.address, challenger1.address, voucher2.address, other.address],
        [],
        []
      );
  ...
  it("Should not use more vouches than needed", async () => {
    ...
    await poh
      .connect(governor)
      .changeStateToPending(requester.address, [voucher1.address, voucher2.address, voucher3.address], [], []);
    await checkSubmissionInfo(voucher1.address).for({ hasVouched: true });
    await checkSubmissionInfo(voucher2.address).for({ hasVouched: true });
  ...
  it("Should allow a mixture of signed and stored vouches", async () => {
    ...
    const vouch = await signTypedData(voucher1, [requester.address, timeout]);
    await poh.connect(voucher2).addVouch(requester.address);

    await poh.connect(governor).changeStateToPending(requester.address, [voucher2.address], [vouch], [timeout]);
  ...
  it("Should set correct values when challenging a removal request", async () => {
    ...
    await poh.connect(requester).removeSubmission(voucher1.address, "", { value: requesterTotalCost });
    await expectRevert(
      poh
        .connect(challenger1)
        .challengeRequest(voucher1.address, Reason.IncorrectSubmission, AddressZero, 0, "", { value: arbitrationCost }),
      "Reason must be left empty"
    );

    await poh
      .connect(challenger1)
      .challengeRequest(voucher1.address, 0, AddressZero, 1, "", { value: arbitrationCost });
    await checkRequestInfo(voucher1.address, 0).for({ currentReason: Reason.None, lastChallengeID: 1, usedReasons: 0 });
  });
  ...
  it("Should set correct values if requester wins removal request", async () => {
    ...
    await poh
      .connect(challenger1)
      .challengeRequest(voucher1.address, Reason.None, AddressZero, 1, "", { value: arbitrationCost });
    await arbitrator.giveRuling(1, Party.Requester);
    await increaseTime(appealTimeOut + 1);
    await arbitrator.giveRuling(1, Party.Requester);

    await checkSubmissionInfo(voucher1.address).for({ status: Status.None, registered: false });
    await checkRequestInfo(voucher1.address, 0).for({ resolved: true });
    await checkChallengeInfo(voucher1.address, 0, 0).for({ ruling: Party.Requester });
  });
  ...
  it("Should set correct values if challenger wins removal request", async () => {
    ...
    await checkSubmissionInfo(voucher1.address).for({ status: Status.None, registered: true });
    await checkRequestInfo(voucher1.address, 0).for({ resolved: true });
    await checkChallengeInfo(voucher1.address, 0, 0).for({ ruling: Party.Challenger });
  });
  ...
  it("Should process vouches correctly", async () => {
    ...
    await checkSubmissionInfo(voucher1.address).for({ hasVouched: false });
    await checkSubmissionInfo(voucher2.address).for({ hasVouched: true });
    await checkSubmissionInfo(voucher1.address).for({ registered: true });

    await poh.connect(governor).processVouches(requester.address, 0, 1);
    await checkSubmissionInfo(voucher2.address).for({ hasVouched: false });
    await checkSubmissionInfo(voucher2.address).for({ registered: true });
  });
  ...
  it("Should correctly penalize vouchers that vote for a bad submission", async () => {
    ...
    await poh.connect(governor).changeRequiredNumberOfVouches(1);
    await poh.connect(voucher1).reapplySubmission("", "", { value: requesterTotalCost });
    await poh.connect(voucher3).addVouch(voucher1.address);
    await poh.connect(governor).changeStateToPending(voucher1.address, [voucher3.address], [], []);
    ...
    await checkRequestInfo(voucher1.address, 0).for({ requesterLost: true });
    await increaseTime(challengePeriodDuration + 1);
    await poh.connect(governor).executeRequest(voucher1.address);
