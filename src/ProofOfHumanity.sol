 ...
 */
 contract ProofOfHumanity is IProofOfHumanity, IArbitrable, IEvidence {
    ...
    struct Humanity {
        ...
        Request[] requests; // Mapping of the ids to corresponding requests.
    }
    ...
    function initialize(
        ...
        _DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                0x8cad95687ba82c2ce50e74f7b754645e5117c3a5bec8151c0726d5857980a866, // keccak256("EIP712Domain(string name,uint256 chainId,address verifyingContract)")
                0x6358f15b5db98e0f8cdb4c73845d2cf46001eb6a51ef69d17fe7ecb32c698593, // keccak256("Proof of Humanity")
                block.chainid,
                address(this)
            )
        );
    ...
    function renewHumanity(string calldata _evidence) external payable {
        ...
        require(humanity.expirationTime.subCap64(renewalPeriodDuration) <= block.timestamp);
    ...
    function fundRequest(bytes20 _humanityId, uint256 _requestId) external payable {
        ...
        uint256 totalCost = _arbitrationCost(arbitratorDataList[request.arbitratorDataId]).addCap(requestBaseDeposit);
    ...
    function advanceState(
        ...
        bytes20 humanityId = humans[claimer];
        Humanity storage humanity = humanityMapping[humanityId];
        uint256 requestId = humanity.claims[claimer];
        Request storage request = humanity.requests[requestId];
        require(request.status == Status.Vouching);
        require(!_humanityClaimed(humanity));
        ...
        while (request.vouches.length < requiredNumberOfVouches) {
            ...
            if (i < _signatureVouches.length) {
                SignatureVouch calldata signature = _signatureVouches[i];
                require(signature.s <= 0x7FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF5D576E7357A4501DDFE92F46681B20A0);

                voucherAccount = _recoverSigner(signature, humanityId, claimer);
                ...
                isValid = vouches[voucherAccount][claimer][humanityId];
            }
        ...
        emit StateAdvanced(claimer);
    }

    function _recoverSigner(SignatureVouch calldata _signature, bytes20 _humanityId, address _claimer) internal view returns (address) {
        return ecrecover(
                    keccak256(
                        abi.encodePacked(
                            "\x19\x01",
                            _DOMAIN_SEPARATOR,
                            keccak256(
                                abi.encode(_IS_HUMAN_VOUCHER_TYPEHASH, _claimer, _humanityId, _signature.expirationTime)
                            )
                        )
                    ),
                    _signature.v,
    ...
    function executeRequest(bytes20 _humanityId, uint256 _requestId) external {
        Humanity storage humanity = humanityMapping[_humanityId];
        Request storage request = humanity.requests[_requestId];
        require(request.status == Status.Resolving, "!resolving");
        require(request.challengePeriodEnd < uint64(block.timestamp), "!time");
    ...
    function withdrawFeesAndRewards(
        ...
        require(request.status == Status.Resolved, "!resolved");
        require(_beneficiary != address(0x0), "!address");
     ...
     */
    function _arbitrationCost(ArbitratorData memory _arbitratorData) internal view returns (uint256) {
    ...
    function _noOngoingClaim(address _account) internal view returns (bool) {
        return humanityMapping[humans[_account]].claims[_account] == 0;
    }
    ...
    function isClaimed(bytes20 _humanityId) external view override returns (bool) {
        return _humanityClaimed(humanityMapping[_humanityId]);
    }
    ...
    function boundTo(bytes20 _humanityId) external view returns (address) {
        Humanity storage humanity = humanityMapping[_humanityId];
        if (_humanityClaimed(humanity)) return humanity.owner;
        return address(0x0);
    }
    ...
    {
        Round storage round = humanityMapping[_humanityId].requests[_requestId].challenges[_challengeId].rounds[_round];
        return (
